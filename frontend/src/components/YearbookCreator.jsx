...
const YearbookCreator = ({ onClose, onSuccess }) => {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      alert("Failed to fetch memories: " + (err.response?.data?.error || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async () => {
    setLoading(true);
    try {
      const activeMemories = previewMemories.filter(m => !removedMemoryIds.has(m.id));
      if (activeMemories.length === 0) {
        alert("At least one memory is required.");
        return;
      }
      ...
      formData.append("memoryIds", JSON.stringify(activeMemories.map(m => m.id)));

      const result = await yearbookService.createPersonalYearbook(formData);
      onSuccess(result.releaseId);
    } catch (err) {
      alert("Failed to create yearbook: " + (err.response?.data?.error || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="scrapbook-page modal-composer profile-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '600px' }}>
        ...
        {step === 1 && (
          <div className="composer-form">
            ...
            <div className="form-group">
              ...
              <select 
                value={form.privacy}
                onChange={e => setForm({...form, privacy: e.target.value, clubCode: ""})}
              >
            ...
            <div className="composer-actions">
              ...
              <button className="primary" onClick={handlePreview} disabled={!form.title || loading}>
        ...
        {step === 2 && (
          <div className="composer-form">
            <p className="composer-hint">
              Found {previewMemories.length} memories. Remove any you don't want in your yearbook.
            </p>
            
            <div className="memories-preview-list" style={{ maxHeight: '400px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <div className="memory-card" style={{ border: '1px dashed var(--tui-col-accent)', opacity: 0.8 }}>
                <h4 className="memory-title">COVER PAGE</h4>
                <p className="memory-text">{form.title}</p>
                {form.coverFile && <p className="composer-hint">Image selected: {form.coverFile.name}</p>}
              </div>

              {previewMemories.map(m => (
                <div key={m.id} className={`memory-card ${removedMemoryIds.has(m.id) ? 'memory-card--removed' : ''}`} style={{ opacity: removedMemoryIds.has(m.id) ? 0.4 : 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <h4 className="memory-title">{m.title || "Untitled Memory"}</h4>
                    <button 
                  ...
                  <p className="memory-text" style={{ fontSize: '0.8rem' }}>{m.content?.slice(0, 100)}...</p>
                </div>
              ))}
            </div>
            ...
            <div className="composer-actions" style={{ marginTop: '1rem' }}>
              ...
              <button className="primary" onClick={handleCreate} disabled={loading || (previewMemories.length - removedMemoryIds.size === 0)}>
