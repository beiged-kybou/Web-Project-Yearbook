...
### `wipe-users.mjs`
Removes users, students, & related memories/tag data. Safe to run repeatedly.
...
also upserts supporting `departments` & `yearbooks` rows.

### `seed-clubs.mjs`
Creates a small set of sample clubs & ensures the required tables exist.

### `seed-minimal.mjs`
Convenience wrapper that runs `seed-clubs` & `create-dummy-users` to get a
